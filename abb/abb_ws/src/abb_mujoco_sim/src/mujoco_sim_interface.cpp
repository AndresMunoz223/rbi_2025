#include "abb_mujoco_sim/mujoco_sim_interface.hpp"

namespace abb_simulator_layer {

hardware_interface::CallbackReturn AbbSimulatorInterface::on_init
(const hardware_interface::HardwareInfo &info){
    ...
    RCLCPP_INFO(rclcpp::get_logger("abb_sim"), "I STARTED");
    return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::CallbackReturn AbbSimulatorInterface::on_configure
...
hardware_interface::CallbackReturn AbbSimulatorInterface::on_deactivate
(const rclcpp_lifecycle::State & previous_state){
...
hardware_interface::return_type AbbSimulatorInterface::read
...
hardware_interface::return_type AbbSimulatorInterface::write
(const rclcpp::Time & time, const rclcpp::Duration & period){
...
AbbSimulatorInterface::export_state_interfaces()
{
...
AbbSimulatorInterface::export_command_interfaces()
{
...
void AbbSimulatorInterface::sim_thread(){
            ...
            int body_id = mj_name2id(this->sim_model_object, mjOBJ_BODY, "sixth_joint_body");
            if (body_id == -1) {
                RCLCPP_ERROR(rclcpp::get_logger("abb_sim"), "En efector '{sixth_joint_body}' not found");
            } else {
                this->sim_cam.type = mjCAMERA_TRACKING;
                this->sim_cam.trackbodyid = body_id;
                RCLCPP_INFO(rclcpp::get_logger("abb_sim"), "Setting camera tracking");
            }
            ...
            glfwTerminate();
            RCLCPP_INFO(rclcpp::get_logger("abb_sim"), "Exit screen");
        }

void AbbSimulatorInterface::start_sim(){
    this->simulation_thread_ = std::thread(&AbbSimulatorInterface::sim_thread,this);
}

AbbSimulatorInterface::~AbbSimulatorInterface(){
    this->cleanup();
    sim_stop_flag = true;
...
void AbbSimulatorInterface::cleanup(){
    mj_deleteModel(this->sim_model_object);
    mj_deleteData(this->sim_data_object);
}
...
PLUGINLIB_EXPORT_CLASS(abb_simulator_layer::AbbSimulatorInterface,hardware_interface::SystemInterface)
