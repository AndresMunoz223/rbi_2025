...
class IkSolverScara():
    def __init__(self, l1, l2, thetha_1_bounds=[62, -62], thetha_2_bounds=[100, -100], z_bounds=[0.3, 0]):
        ...
        ic(f"Robot configured with : {self.l1, self.l2, self.thetha_1_max, self.thetha_2_max}")
    ...
    def solve_for_pose(self, pose : np.array):
        ...
        thetha_2 = np.arctan2(np.sqrt(1 - d**2)/d)
        theta_1 = np.arctan2(y/x) - np.arctan2(np.sin(theta_2)*self.l2/(self.l1 + np.cos(thetha_2)*self.l2)) 
        ...
        return np.array([[theta_1], [thetha_2], [z], [phi]])


class IkSolverNode(Node):
    def __init__(self):
        super().__init__("ik_solver_node")
        self.declare_parameter("link_1_lenght", 1.0)
        self.declare_parameter("link_2_lenght", 1.0)
        ...
        self.ik_solver = IkSolverScara(l1_lenght, l2_lenght)
        
        self.pose_subs = self.create_subscription(PoseStamped, "/scara/goal_pose", self.goal_pose_callback, 10)
        self.joint_pub = self.create_publisher(Float32MultiArray, "/arm_joints_controller/commands", 10)
        
    
    def goal_pose_callback(self, msg : PoseStamped):
        
        roll, pitch, yaw = [msg.pose.orientation.x, msg.pose.orientation.y, msg.pose.orientation.z, msg.pose.orientation.w]
        
        desired_pose = np.array([[msg.pose.position.x], [msg.pose.position.y], [msg.pose.position.z], [yaw]])
        
        joint_coords = self.ik_solver(desired_pose)
